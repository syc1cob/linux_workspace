...
int main(int argc, char *argv[])
{
    ...
    /* Purpose of bind() system call is that application() inform the underlying 
     * operating system the criteria of recieving the data. Here, bind() system call
     * is telling the OS that if sender process sends the data destined to socket "/tmp/mySocket", 
     ...
     * to 25. So while one request is being processed other requests
     * can be waiting.
     * */

    ret = listen(connection_socket, 25);
    if (ret == -1) {
        perror("listen");
        exit(EXIT_FAILURE);
    }
    ...
    /*All Server process continuously*/
    while (1) {
